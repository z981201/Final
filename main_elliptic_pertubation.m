...
gamma_dt2 = {@(t) -1.*a*cos(t);
            @(t)  -1.*b*sin(t)+3*alpha*(2*sin(t).*cos(t).^2-sin(t).^3)}; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 3. Plot
gamma_plot = @(x,y) x.^2/a^2+y.^2/b^2-2*alpha/b*(1-x.^2/a^2).^2-alpha^2/b^2*(1-x.^2/a^2).^3;
axis_plot = [-a a;-b-alpha b+alpha];
N_plot = [500,500];
...
N_box = floor(N/5)+1; % num of box
p = 15; % Taylor num

theta_z0 = (0:N_box-1)'/N_box*2*pi; % z0
z0 = gamma{1}(theta_z0,1-2.5*h)+sqrt(-1)*gamma{2}(theta_z0,1-2.5*h);
