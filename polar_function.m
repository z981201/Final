function [target,u_correction,cost_time] = polar_function(N,m,k)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 0. Initialize
% 0.1 Gamma
tic
Gamma_rho = @(t) 1+ cos(k*t)/(k+1);
Gamma_rho_dt = @(t) -k/(k+1)*sin(k*t);
Gamma_rho_dt2 = @(t) -k^2/(k+1)*cos(k*t);
a = 1+1/(k+1);
b = 1+1/(k+1);

gamma = {@(t) Gamma_rho(t).*cos(t);
         @(t) Gamma_rho(t).*sin(t)}; 
gamma_dt = {@(t) Gamma_rho_dt(t).*cos(t)-Gamma_rho(t).*sin(t);
            @(t) Gamma_rho_dt(t).*sin(t)+Gamma_rho(t).*cos(t)}; 
gamma_dt2 = {@(t) Gamma_rho_dt2(t).*cos(t)-2*Gamma_rho_dt(t).*sin(t)-Gamma_rho(t).*cos(t);
            @(t)  Gamma_rho_dt2(t).*sin(t)+2*Gamma_rho_dt(t).*cos(t)-Gamma_rho(t).*sin(t)}; 
...
% 0.4 target in Omega
rho_correct = 0.1; % correct by power function
rho_correct = ((1:m)/(m+1)).^(1/(1+rho_correct));
% rho_correct = (N-1)/N; % rand and correct by linear
% rho = rand(1,m)*rho_correct;
target = Gamma_rho(theta)*rho_correct;
target = [target.*cos(theta) target.*sin(theta)];
...
% 1.2 A
eta = [gamma_dt{2}(theta) -gamma_dt{1}(theta) ];
...
gamma_plot = @(x,y) sqrt(x.^2+y.^2) ;
for i=0:k
    gamma_plot = @(x,y) gamma_plot(x,y)-chebshev(i+1)*(x./sqrt(x.^2+y.^2)).^i/(k+1);
end
% 4.1 partition
rho_reduced = (1-6*h)^2;
...
N_box = floor(N/3)+1; % num of box
p = 15; % Taylor num

theta_z0 = (0:N_box-1)'/N_box*2*pi; % z0
z0 = (1-4*h)*Gamma_rho(theta_z0).*exp(sqrt(-1)*theta_z0);
% z0 = [z0; (1-5*h)*Gamma_rho(theta_z0).*exp(sqrt(-1)*theta_z0)];
...
nu = gamma_dt{1}(theta_Taylor)+sqrt(-1)*gamma_dt{2}(theta_Taylor);
nu = nu.*mu_spline;
